...
def singleFlow(jobs: Map[String, String]): Unit =
  ...
  * */
  val ((mqttSendQueue, subscribed), mqttReceiveQueue) = Source.queue(100)
    ...
    .toMat(Sink.foreach(
      m => handleNewMessage(m) //println(m.topic+m.payload.decodeString("utf-8"))
    ))(Keep.both)
    .run()
...
def handleNewMessage(msg: MqttMessage): Unit =
  val payload = msg.payload.decodeString("utf-8")
  msg.topic match
    case "results" => printResults(payload)
    case "new_client" => handleConnection(payload)

def printResults(res: String): Unit =
  println("\nNew results!\n" + res)

def handleConnection(id: String): Unit =
...
@main def hello(): Unit =
  ...
  val unitCount = 1 //scala.util.Properties.envOrElse("UNIT_COUNT", "undefined")
  val filename = "/library/Bible.txt" //scala.util.Properties.envOrElse("FILENAME", "undefined")
  val regex = "God.{5}" //scala.util.Properties.envOrElse("REGEX", "undefined")
  ...
  val splitting = computeSplitting(lines, unitCount)
    .map((k,v)=>{
      k-> (filename+":::"+regex+v)
    })
